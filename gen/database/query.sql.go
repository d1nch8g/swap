...
func (q *Queries) GetBalanceById(ctx context.Context, arg GetBalanceByIdParams) (Balance, error) {
	row := q.db.QueryRow(ctx, getBalanceById, arg.ID, arg.UserID)
	var i Balance
	err := row.Scan(
		&i.ID,
		&i.UserID,
...
const getFreeOperators = `-- name: GetFreeOperators :many
...
func (q *Queries) GetFreeOperators(ctx context.Context) ([]User, error) {
	rows, err := q.db.Query(ctx, getFreeOperators)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const removeBalance = `-- name: RemoveBalance :exec
...
func (q *Queries) UserOrders(ctx context.Context, userID int64) ([]Order, error) {
	rows, err := q.db.Query(ctx, userOrders, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
